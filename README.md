...
1. **`❌ ไม่พบข้อมูลยืนยันตัวตนในระบบ` (No Auth Context)**
   - **Cause:** The request header (e.g., `x-user-email` or `current-user`) or JWT token was not sent.
   - **Solution:** Ensure your client/front-end forwards user information in headers or includes a valid JWT token.
