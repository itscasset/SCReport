...
# --------------------------------------------------------
# ✅ MCP Server — Stateless HTTP (Option A)
#    - stateless_http=True  → ไม่ต้องการ session_manager lifecycle
#    - mount ที่ /mcp        → ไม่ชน FastAPI routes
#    - ลบ startup/shutdown  → ไม่จำเป็น และเป็นสาเหตุของ crash
# --------------------------------------------------------
mcp = FastMCP(
    "sc-report-server",
    stateless_http=True,
...
# ✅ Mount ที่ /mcp ไม่ใช่ "" เพื่อไม่ให้ชน FastAPI routes
...
def debug_check_permission(request: GenerateReportRequest, http_request: Request):
    ...
    clean = table_id.split(".")[-1].lower()
    report_names = TABLE_TO_REPORT_NAMES.get(clean)
    result = check_user_permission(user_email, table_id)

    return {
        "received_email": user_email,
        "received_table_id": table_id,
